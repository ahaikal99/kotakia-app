            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-8">
                @forelse ($designs as $design)
                    <article class="catalog-card group rounded-[2rem] border border-slate-200 bg-white p-3 shadow-sm transition hover:shadow-xl hover:border-[#B6975A]/50">
                        <a href="{{ $design->previewUrl() }}" target="_blank" rel="noopener" class="catalog-art block relative overflow-hidden rounded-[1.5rem] bg-[#f1eee8]" aria-label="Preview {{ $design['name'] }}, {{ $design['code'] }} (tab baharu)">
